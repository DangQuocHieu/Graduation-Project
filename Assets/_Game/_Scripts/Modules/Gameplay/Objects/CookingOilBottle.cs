...
public class CookingOilBottle : GrabbableObject
{
    ...
    private IEnumerator InteractWithFryingPanCoroutine(FryingPan fryingPan, PickupAndDropHandler pickupAndDropHandler)
    {
        ...
        transform.DOMove(fryingPan.oilContainer.pourPoint.position, 0.5f).SetEase(Ease.InOutSine).SetLink(gameObject);
        yield return transform.DORotate(new Vector3(0, 0f, 180f), 0.5f, RotateMode.LocalAxisAdd).SetEase(Ease.InOutSine).SetLink(gameObject).WaitForCompletion();
        pourEffect.Play();
        ...
        yield return topNap.DOLocalRotate(new Vector3(0f, 0f, 0f), 0.3f, RotateMode.LocalAxisAdd).SetEase(Ease.InOutSine).SetLink(gameObject).WaitForCompletion();
